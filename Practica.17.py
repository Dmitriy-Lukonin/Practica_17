...
try:
    while in_number.isalpha() or int(in_number) < 0:
        in_number = input("Введено не число, введите число: ")
except ValueError:
    in_number = input("Введено не число, введите число: ")
...
while True:
    ...
    if not str_list.isdigit():
        print("Ошибка при вводе последовательности\n"
              "Допускается ввод чисел через пробел")
    else:
        list_array = entered_list.split()
        break


# Быстрая сортировка списка по возрастанию элементов нем
arr = [int(item) for item in list_array]  # - преобразование введённой последовательности в список
